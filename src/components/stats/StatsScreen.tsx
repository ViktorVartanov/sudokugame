    ...
    <div className="min-h-screen bg-noise">
      <div className="mx-auto max-w-5xl px-4 pb-16 sm:px-8">
        <header className="flex items-center justify-between py-6 sm:py-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">{t('stats.subtitle')}</p>
            </div>
          </div>
          <ThemeToggle />
        </header>
        ...
        <section className="grid grid-cols-2 gap-3.5 sm:grid-cols-3 sm:gap-5 lg:grid-cols-6">
          ...
          <StatCard
            icon={<Target size={20} />}
            label={t('stats.perfectSolves')}
            value={perfectSolves}
            sublabel={t('stats.perfectSolvesSub')}
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {isCompleted ? (
                      <p className="text-xs text-slate-400 dark:text-slate-500">
                        {t('stats.solvedCount', {
                          count: progress!.timesCompleted,
                          time: formatTime(progress!.bestTimeSeconds ?? 0),
                        })}
                      </p>
                    ) : (
                      <p className="text-xs text-slate-400 dark:text-slate-500">{t('stats.notSolvedYet')}</p>
                    )}
                  </div>
