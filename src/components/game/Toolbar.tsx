...
function ToolbarButton({ icon, label, badge, active, disabled, onClick }: ToolbarButtonProps) {
  return (
    <button
      ...
      disabled={disabled}
      className={cn(
        'toolbar-action relative flex flex-1 flex-col items-center gap-1 rounded-2xl px-2 py-2.5 text-slate-600 transition-all active:scale-95 disabled:opacity-30 dark:text-slate-300',
        active
          ? 'toolbar-action-active bg-brand-100 text-brand-700 dark:bg-brand-500/20 dark:text-brand-300'
...
export function Toolbar() {
  ...
  return (
    <div className="premium-toolbar mx-auto flex w-full max-w-[min(92vw,32rem)] gap-1 rounded-2xl bg-white p-1.5 shadow-sm ring-1 ring-slate-200 dark:bg-slate-800/60 dark:ring-slate-700">
